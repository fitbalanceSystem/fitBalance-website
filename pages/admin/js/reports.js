...
async function loadStatusReport() {
  ...
  const [{ data: customers }, { data: enrollments }, { data: trials }] = await Promise.all([
    supabaseClient.from('customers').select('id, status_code'),
    supabaseClient.from('program_enrollments').select('customer_id, start_date, end_date'),
    supabaseClient.from('session_attendance').select('customer_id, session_id, is_present').eq('status_code', 3),
  ]);
...
async function loadDebtReport() {
  ...
  const [{ data: customers }, { data: enrollments }, { data: payments }] = await Promise.all([
    supabaseClient.from('customers').select('id, firstName, lastName, mobile'),
    supabaseClient.from('program_enrollments').select('id, customer_id, start_date, end_date, programs!fk_enrollments_program(name, price)'),
...
async function loadTrialsReport() {
  ...
  const { data: trials, error } = await supabaseClient.from('session_attendance')
    .select(`
  ...
  let enrollments = [];
  if (custIds.length) {
    const { data } = await supabaseClient.from('program_enrollments').select('customer_id, start_date, end_date').in('customer_id', custIds);
    enrollments = data || [];
  }
  tbody.innerHTML = '';
...
async function loadSalaryReport() {
  ...
  .or('status.is.null,status.neq.2'),
    supabaseClient.from('instructors')
...
async function loadAttendanceReport() {
  ...
  const [{ data: sessions, error }, { data: instructors }] = await Promise.all([
    ...
    supabaseClient.from('instructors').select('id, firstName, lastName'),
  ]);
  if (error) { tbody.innerHTML = `<tr><td colspan="6" class="text-center p-4 text-red-500">שגיאה</td></tr>`; return; }
...
async function loadManagementReport() {
  ...
  const [{ data: enrollments }, { data: payments }] = await Promise.all([
    supabaseClient.from('program_enrollments').select(`
  ...
  const { data: monthPayments } = await supabaseClient.from('payments')
...
async function runCsvCompare() {
  ...
  const { data: enrollments } = await supabaseClient.from('program_enrollments').select(`
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // טעינת מדריכות לסלקט
  const { data: instructors } = await supabaseClient.from('instructors').select('id, firstName, lastName').order('firstName');
...
async function generateAttendancePdf() {
  ...
  const { data: programs, error: progErr } = await supabaseClient.from('programs').select('id, name, day, time').order('day');
  if (progErr) { statusEl.textContent = 'שגיאה: ' + progErr.message; return; }

  const { data: sessions } = await supabaseClient.from('program_sessions')
  ...
  const { data: enrollments } = await supabaseClient.from('program_enrollments')
